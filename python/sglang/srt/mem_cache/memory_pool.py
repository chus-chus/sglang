...
class MHATokenToKVPoolHost(HostKVCache):
    ...
    def load_page_per_layer(self, host_indices, device_indices, device_pool, layer_id):
        ...
        for i in range(len(device_indices_cpu)):
            h_index = host_indices[i * self.page_size]
            d_index = device_indices_cpu[i]
            device_pool.k_buffer[layer_id - device_pool.start_layer][
                d_index : d_index + self.page_size
            ].copy_(
                self.kv_buffer[
                    0, layer_id - device_pool.start_layer, h_index : h_index + self.page_size
                ],
                non_blocking=True,
            )
            device_pool.v_buffer[layer_id - device_pool.start_layer][
                d_index : d_index + self.page_size
            ].copy_(
                self.kv_buffer[
                    1, layer_id - device_pool.start_layer, h_index : h_index + self.page_size
                ],
                non_blocking=True,
            )
